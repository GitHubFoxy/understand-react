...
const LayoutEffectComponent = () => {
    ...
    return (
        ...
        <div className="flex gap-2">
            <div>
            <p>useEffect</p>
            <button className="p-2 px-4 bg-slate-500 hover:bg-slate-600 transition-colors whitespace-nowrap	"onClick={() => setNumber(0)}>Сгенироривать число</button>
            <p>number - {number}</p>
            <p>Видно что строчка дергается, потому что на 0.1 секунду результат 0</p>
            </div>
            <div>
            <p>useLayoutEffect</p>
            <button className="p-2 px-4 bg-slate-500 hover:bg-slate-600 transition-colors whitespace-nowrap	" onClick={() => setCoolNumber(0)}>Сгенироривать число</button>
